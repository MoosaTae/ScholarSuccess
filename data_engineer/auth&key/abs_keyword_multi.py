...
class ScopusDataScraper:
    def __init__(
        self, base_dataset_path: str, output_path: str, headers: Dict[str, str]
    ):
    ...
    def process_paths(self, paths: List[str] = None, num_threads: int = 4):
        if not paths:
            paths = [
                p
                for p in os.listdir(self.base_dataset_path)
        ...
        for chunk in file_chunks:
            thread = threading.Thread(
                target=self._process_file_chunk, args=(chunk, progress_bar)
            )
            thread.start()
    ...
    def _process_file(self, file: str, data: List[Dict]):
        ...
        for row in tqdm(data):
            try:
                url = (
                    f"https://www.scopus.com/gateway/doc-details/documents/{row['eid']}"
                )
                ...
                if response.status_code != 200:
                    with open("error.txt", "a") as f:
                        f.write(
                            f"Error processing {row.get('eid', 'unknown')}: {response.status_code}\n"
                        )
                    continue
                ...
                with self.lock:
                    row["abstract"] = (
                        response_json.get("abstract", [None])[0]
                ...
                output_file = os.path.join(
                    self.output_path, f"{file.split('.')[0]}_add.json"
                )
...
def main():
    ...
    scraper.process_paths(num_threads=1)
